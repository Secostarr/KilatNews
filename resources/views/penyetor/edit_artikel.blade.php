...
<body>
    <div class="container my-5 card p-3">
        <div class="d-flex align-items-center mb-3">
            ...
            <h2 class="m-0">Edit Artikel Contributor</h2>
        </div>
        @if(session('success'))
        ...
        @endif
        <hr class="my-4">
        <form action="{{ Route('contributor.dashboard.update', ['id_artikel' => $artikel->id_artikel]) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row g-4">
                <!-- Kolom Kiri -->
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="kategori" name="id_kategori">
                            ...
                            <option value="{{ $kategori->id_kategori }}" {{ $kategori->id_kategori == $artikel->id_kategori ? 'selected' : '' }}>{{ $kategori->nama_kategori }}</option>
                            @endforeach
                        </select>
                        @error('id_kategori')
                        <div class="text-danger">{{ $message }}</div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="highlight" name="highlight">
                            <option value="1" {{ old('highlight', $artikel->highlight ?? '') == 1 ? 'selected' : '' }}>Iya</option>
                            <option value="0" {{ old('highlight', $artikel->highlight ?? '') == 0 ? 'selected' : '' }}>Tidak</option>
                        </select>
                        @error('highlight')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-select" id="trending" name="trending">
                            <option value="" disabled {{ old('trending', $artikel->trending ?? '') === '' ? 'selected' : '' }}>Pilih Trending</option>
